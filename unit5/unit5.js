...
// В каждой итерации цикл выводит 6 звездочек. 
// Перенос строки - br. 
...
// Разделитель - знак подчеркивания. 
...
// которая выводит в .out-8 числа от первого введенного до второго включительно, 
// с шагом 1.
...
function t8() {
    let input1Value = +document.querySelector('.i-81').value,
        input2Value = +document.querySelector('.i-82').value,
        out = document.querySelector('.out-8'),
        temp = '';

    for(let i = input1Value; i <= input2Value; i++) {
        temp += i + '_';
    }
...
function t9() {
    let input1Value = +document.querySelector('.i-91').value,
        input2Value = +document.querySelector('.i-92').value,
        out = document.querySelector('.out-9'),
        temp = '';

    if(input1Value > input2Value) {
        for(let i = input2Value; i <= input1Value; i++) {
            temp += i + '_';
        }
    } else {
        for(let i = input1Value; i <= input2Value; i++) {
...
// Функция должна выводить в .out-10 четные годы от 1950 до 1970 включительно.
// Разделитель - знак подчеркивания. 
// Задача решается через цикл, а четность - через шаг (равный 2).
...
function t12() {
    let elements = document.querySelectorAll('div.div-12');
...
function t14() {
    let elements = document.querySelectorAll('input.i-14'),
        out = document.querySelector('.out-14');

    for(let i = 0; i < elements.length; i++) {
        if(elements[i].checked) {
            out.innerHTML = elements[i].value;
            break;
        }
    }
}
