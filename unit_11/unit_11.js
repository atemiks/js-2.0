...
function f12() {
    let index = -1,
        inputValue = +document.querySelector('.i-12').value;
...
function f13() {
    ...
    for(let i = d13.length - 1, k = 0; i >= 0; i--, k++ ) {
        tempArray[k] = d13[i];
    }
...
// Т.е. пользователь ввел 5, массив будет [1,1,1,1,1]. 
...
// Task 16
// Даны два массива d161 и d162 - используя метод concat 
// - добавьте в массив d16 результат применения concat между массивом d161 и d162.
// Во всех задачах где говорим второй, третий и т.д. имеем ввиду индекс
...
// и проверяет есть ли такое значение в массиве d18 c использованием метода include. 
...
function f18() {
    ...
    document.querySelector('.out-18').innerHTML = d18.includes(inputValue);
}
...
// Task 20
// Напишите функцию f20, которая применяет к массиву d20 метод join с параметрами 
// и соединяет элементы массива в одну строку. 
// Результат выводит в .out-20. 
// Укажите мето так d20.join(''). 
...
function f20() {
    let tempArray = d20.join('')

    document.querySelector('.out-20').innerHTML = tempArray;
}
